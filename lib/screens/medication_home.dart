...
import 'package:smartpillbox_alarm/credential.dart';
...
class MedicationAlarmHomeScreen extends StatefulWidget {
  ...
  @override
  State<MedicationAlarmHomeScreen> createState() => _MedicationAlarmHomeScreenState();
}
...
class _MedicationAlarmHomeScreenState extends State<MedicationAlarmHomeScreen> {
  ...
  Widget _buildTestButton(String timeSlot){
    return ElevatedButton(
      onPressed: () async{
        int alarmId=await MedicationNotificationService.testNotification(timeSlot);
  ...
  Future<void> _syncMedicationsToAlarms() async{
    ...
    try{
      List<int> createdAlarms=await MedicationNotificationService.updateMedicationAlarms(medications);
      await loadAlarms();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('已同步 ${createdAlarms.length}個藥物提醒'),),
      );
    }catch(e){
      ScaffoldMessenger.of(context).showSnackBar(
